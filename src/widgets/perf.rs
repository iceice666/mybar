...
pub fn measure(fc: &FontCollection, data: &BarData) -> f32 {
    let icon_w = measure_text(fc, ICON_CPU, style::FONT_FAMILY_TEXT, style::FONT_SIZE_2XL);

    let cpu_label_w = measure_text(fc, "CPU", style::FONT_FAMILY_TEXT, style::FONT_SIZE_2XS);
    let cpu_val = format!("{:>3.0}%", data.cpu_percent);
    let cpu_val_w = measure_text(fc, &cpu_val, style::FONT_FAMILY_TEXT, style::FONT_SIZE_SM);
    ...
    let mem_val_w = measure_text(fc, &mem_val, style::FONT_FAMILY_TEXT, style::FONT_SIZE_SM);
...
pub fn draw(canvas: &Canvas, fc: &FontCollection, data: &BarData, rect: Rect) {
    ...
    // CPU icon
    let icon_h = text_height(fc, ICON_CPU, style::FONT_FAMILY_TEXT, style::FONT_SIZE_2XL);
